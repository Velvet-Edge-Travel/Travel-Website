...
const generateHtml = (data, preview = false) => {
  const pageBase = preview
    ? `<base href="${attributeEscape(new URL(".", window.location.href).href)}">`
    : "";
...
<head>
  ...
  ${pageBase}<link rel="stylesheet" href="styles.css">
</head>
<body class="quote-page">
  <header class="quote-header"><div class="container quote-header__inner">
    <a class="brand" href="index.html"><img class="brand__mark" src="velvet-edge-logo-v3.svg" alt="" width="150" height="56"><span class="brand__name">Velvet Edge Travel</span></a>
...
document.querySelector("[data-preview]").addEventListener("click", () => {
  ...
  const url = URL.createObjectURL(
    new Blob([generateHtml(getData(), true)], { type: "text/html" }),
  );
...
form.addEventListener("submit", (event) => {
  ...
  download(generateHtml(data), filename, "text/html");
  setStatus(
  ...
  .addEventListener("change", async (event) => {
    ...
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (key in form.elements && typeof value === "string") {
